...
namespace CRT571
{
    public class CardDispenser
    {
        ...
        public CardDispenser(EquipmentEntity equipment, int restTimeSec = 20)
        {
            try
            {
                _ip = equipment.Ip;
                ...
                AddLog("          **********          START");
                AddLog($"CardDispenser --> IP:{_ip} Port:{_port} ComPort:{_comport} RestTimeSec:{_restTimeSec}");
            }
            catch (Exception ex)
            {
        ...
        public CardDespenserResult OpenCardDespenser()
        {
            ...
            try
            {
                ...
                if (Hdle != 0)
                {
                    res.MessageText += "Comm. Port is Opened";
        ...
        private CardDespenserResult CardOut()
        {
                            ...
                            case 0x4d:  //mifare one Card
                                {
                                    ...
                                    cardDespenserResult.CardID = CardUID;
        ...
        private  void ProcessPressButton()
        {
            ...
            if (diffInSeconds >= _restTimeSec)
            {
                ...
                if (!String.IsNullOrEmpty(res.CardID))
                {
                    PrintCardID(res.CardID);
                }
                else
                {
                    AddLog($"FaildCardOut --> {res.MessageText}");
        ...
        private void PrintCardID(string cardID)
        {
            //send to PRINT
